document.addEventListener("DOMContentLoaded", () => {
  const baseURL = "https://dr7kat5az5.execute-api.us-east-1.amazonaws.com/prod"; // Updated API Gateway base URL
  ...
  const setupPasswordToggles = () => {
    document
      ...
      .forEach((checkbox) => {
        const inputWithIconDiv = checkbox.closest(".input-with-icon");
        if (!inputWithIconDiv) return;

        const input = inputWithIconDiv.querySelector(
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (form === forms.login) {
          ...
          localStorage.setItem("userId", result.user.userId); // Store userId
  ...
  const init = () => {
    ...
    attachFormHandler(forms.register, "/register", "POST"); // Changed endpoint to /register
